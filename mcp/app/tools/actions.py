...
async def edit_scheduled_workout(
    ...
    updated version in WorkoutKit on the Apple Watch.
    ...
    Args:
        ...
        blocks: Updated interval blocks (same structure as create_workout).
...
@actions_router.tool
async def batch_actions(actions: list[dict]) -> dict | list:
    """
    ...
    Args:
        actions: List of action objects. Each must have:
            - workoutId (str): UUID of the workout on Apple Watch.
            - action (str): "edit" or "delete".
            - composition (dict|null): Required for "edit" (full workout composition
              with id matching workoutId), null/omitted for "delete".

    Example:
        actions: [
            {"workoutId": "aaa-...", "action": "delete"},
            {"workoutId": "bbb-...", "action": "edit", "composition": {
                "id": "bbb-...", "displayName": "Updated Run", "activityType": "running",
    ...
    """
    try:
        return await client.create_actions_batch(actions)
    except Exception as e:
