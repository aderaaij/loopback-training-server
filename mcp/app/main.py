...
mcp = FastMCP(
    ...
    - get_coaching_playbook: REQUIRED before creating a training plan, materially
      revising one, or advising on training structure/load. Returns the coaching
      methodology this system follows (evidence-based principles + a module for
      the athlete's goal and experience level). Pass goal/experience when known.
      Skip it for simple data questions.
    ...
    - validate_plan: Deterministic soundness check of the upcoming schedule
      (ramp vs real 4-week baseline, down weeks, long-run share, hard-day
      spacing, taper, guardrails). Run after queueing/reshuffling a plan;
      resolve critical warnings before presenting the plan as final.
      create_workout / batch_create_workouts also return these warnings
      inline as `validation` — always read them.
    ...
    2. Use batch_create_workouts (or create_workout) with plan_id set on each workout
       to link them to the plan. Read the `validation` warnings in the response;
       fix critical ones (or get the athlete's sign-off) before calling the plan done.
    ...
    - Both sides carry error, and it does not cancel: intake is self-reported
      and skews low, which biases the computed deficit LARGER, while basal is
      HealthKit's estimate rather than a measurement. A balance is a direction
      with a magnitude attached, not a measured quantity — don't budget an
      exact intake target off one.
    - Scale weight is the only number here that isn't modelled, so it is the
      useful cross-check on a computed balance. Read it from `body`:
      `weight_change` is a least-squares fit across the readings' span (NOT
      last-minus-first, which would report the gap between two arbitrary
      weigh-in conditions as a trend), `weight_sd` is the scatter that fit sits
      in, and `weigh_ins` is how many readings back both.
    ...
    - ⚠️ ONLY FOUR FIELDS ARE DAILY TOTALS: `energy_kcal`, `carbs_g`,
      ...
      other nutrients on only a fraction, so those fields sum just the foods
      that happened to record them. This applies to ALL of:
        `sodium_mg`, `fiber_g`, `sugar_g`, `saturated_fat_g`,
        `cholesterol_mg`, `potassium_mg`, and every key in `micros`.
      Nothing in the payload marks them — no flag, no null — so you must apply
      this from here. A plausible-looking value is not evidence of
      completeness: sodium reading 3,000 mg means "at least 3,000 mg from the
      entries that reported it", not "3,000 mg eaten".
    - NEVER DIAGNOSE A DEFICIENCY OR AN EXCESS FROM A LOWER BOUND. Observed
      here: potassium ~200 mg and iron ~1.6 mg on a 2,600 kcal day, roughly a
      tenth of any plausible intake, purely because most entries contributed
      nothing. "Your potassium is dangerously low" would have been wrong and
      alarming. You may say a lower bound is *at least* X, or that intake is
      *at least* adequate when the bound already clears a target — that
      direction is sound. The reverse never is.
    - Trends in a lower bound are weak evidence too: they move with logging
      detail (which foods the athlete picked from the database) as much as with
      diet. Say so rather than reading a decline as a dietary change.
