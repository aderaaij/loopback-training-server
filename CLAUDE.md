...
- MCP instructions changed shape here: the old blanket *"do not compute an energy balance and present it as fact"* is replaced by a calibrated rule — report **direction, not a figure**; under-reported intake biases the computed deficit **larger**, so the errors compound rather than cancel; and **scale weight over 3–4 weeks is the arbiter**, being the one number in the block that isn't modelled. When weight and balance disagree, the balance is what's wrong.
