...
**Charts are hand-rolled SVG in `components/charts.tsx`** (no charting library), and
every one stretches a fixed viewBox to the container with `preserveAspectRatio="none"`.
⚠️ **That means viewBox units are NOT screen pixels** — a pointer event's SVG coordinate
is meaningless, so `useChartHover` resolves the pointer against the *wrapper element's*
rect and hands back a data index the chart maps to its own geometry. Hover is one
primitive (`useChartHover` + `ChartTooltip` + `TipRow` + `Crosshair`), not one per chart;
`'band'` mode for bars, `'point'` mode for lines/dots. Three things worth not
re-deriving: (1) the tooltip **pins to the top corner away from the mark** rather than
tracking it — centred on the mark it covers the mark whenever the card is short (150px
card vs ~140px tooltip), and a tracking tooltip has no width it can clamp itself to
without measuring, so it overflows on a narrow screen; (2) **`useChartHover` must be
called before a chart's early `return null`**, or the hook count changes with the data
and React throws; (3) sparse series (weight) snap to the nearest real reading resolved
in **full-day index space** — indexing only the days that have a reading spaces them
evenly, which they are not, and the marker drifts off the line. Don't add SVG `<title>`
tooltips back: they're a slow one-line native popup that never fires on a line chart's
individual points, which is what this replaced.
...
- **WorkoutQueue** - structured workouts queued for Apple Watch sync (status: pending/fetched/synced/completed/skipped). Posting missed-workout feedback with `action: "skip"` retires the queue item to `skipped` (feedback `workoutId` == queue item id): the watch endpoints stop serving it and it no longer counts as a schedule collision. One-way; never downgrades `completed`. `scheduled_date` is a first-class indexed column (kept in sync with `workout_data.scheduledDate`, which the iOS app still reads) so the schedule is queryable and can be conflict-checked. ⚠️ **`synced` is per account, not per device:** once any install confirms an item, `GET /api/workouts/queue` never offers it again, so a reinstall or a new phone restores its watch from `GET /api/workouts/queue/scheduled?from=` (fetched/synced items due from `from` on) by diffing against its WorkoutKit schedule (0.1.14). Anything that should stay off the watch must therefore leave those statuses (skip → `skipped`, acked coach delete → `skipped`); an item left `synced` comes back. Feedback `action: "move"` re-dates the item (column + `workout_data.scheduledDate`) so the restore puts it on the new day.
...
- The energy-out side of the Lose It!-style "leeway" question. `daily_health_metrics` holds **`active_energy_burned`** (movement + exercise, already synced) and **`basal_energy_burned`** (resting, added 2026-07-31, migration `a7b8c9d0e1f2`). **TDEE = active + basal, full stop** — `active_energy_burned` already includes workout calories, so adding `workout.total_energy_burned` on top double-counts every session. **The app shipped the basal sync and backfilled** (`docs/app-basal-energy-handoff.md`): as of 2026-08-01 this DB has basal on 367 of 375 days, everything from 2025-07-31 on — only the 8 oldest days (2025-07-23…30) lack it. Code still degrades to active-only where basal is missing, but that path is now the exception, not the state of the world. ⚠️ **Do not "fall back to active alone" when presenting a balance** — active is roughly a third of TDEE here (~1,200 vs ~3,300 kcal), so pairing intake against it invents a ~2,000 kcal deficit out of a missing column. No TDEE → no balance for that day.
...
- **Dashboard surface (0.1.12):** the Health screen's Nutrition block leads with an **"Energy in vs out"** chart (`NutritionCards.tsx`), and the week-by-week table carries **In / Out / Balance**. It is a **bullet chart** — burn is the container, food is how much of it got filled — because the first version shaded the difference as its own block and that reads as a second *stacked* segment, i.e. as something added to intake rather than the room left above it. Colour marks direction only (`ENERGY_IN` warm / `ENERGY_OUT` cool, validated as a pair): **deficit is not green**, since for a runner under-fuelling is not a win. ⚠️ **The table's Balance column is NOT `In − Out` of the two cells beside it** — it is averaged over paired days only, which is why it carries its own `·Nd` day count; expect them to disagree and don't "fix" it.
