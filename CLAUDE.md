...
- ⚠️ **`micros` and `potassium_mg` are sparse sums, not intake — verified against real Lose It! data 2026-07-30.** Food-logging databases carry energy/macros on nearly every entry but micronutrients on only a fraction, so a day's micro columns total just the foods that happened to have the field: potassium ~23–206 mg and iron 0.2–2.8 mg on 1,100–2,600 kcal days, i.e. ~5–12% of any plausible intake (206 mg potassium is under half a banana). Macros are sound over the same rows — macro-derived kcal reconciles with reported kcal within ±11%, normal food-DB noise. So **energy, carbs, protein, fat, saturated fat, fiber, sugar, sodium are trustworthy; treat everything else as a floor of unknown tightness and never infer a deficiency from it.** The MCP instructions say this explicitly because an LLM reading those numbers straight would report a medical emergency. `water_ml`/`caffeine_mg` are usually null — Lose It! doesn't write them.
